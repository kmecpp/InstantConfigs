...
import com.kmecpp.effortlessconfigs.api.ConfigWriteException;
import com.kmecpp.effortlessconfigs.api.Configs;
import com.kmecpp.effortlessconfigs.util.IOUtil;
...
public class ConfigFormatWriter {
	...
	private void writeBlock(Block block) {
		if (!block.isRoot()) {
			sb.append('\n');
			sb.append(tab);
			Configs.writeKey(sb, block.getName());
			//HOCON formatting
		...
		for (ConfigField field : block.getFields()) {
			if (field.getType().toGenericString().contains("<") && (field.getComponentTypes().length == 0 || field.getComponentTypes()[0] == Object.class)) {
			...
			sb.append(tab);
			Configs.writeKey(sb, field.getName());
	...
	private void writeValue(Class<?> type, Object value) {
		...
		//						continue;
		//					}
		//					writeMapElement(field.getType(), Configs.getKey(field.getName()), field.get(value));
